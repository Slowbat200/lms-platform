...
import { revalidateUserCourseAccessCache } from './cache/user-course-access';
...
export async function revokeUserCourseAccess(
  ...
  const validPurchase = await trx.query.PurchaseTable.findMany({
    where: and(
  ...
  const validCourseIds = validPurchase.flatMap((p) =>
  ...
  const revokeAccesses = await trx
